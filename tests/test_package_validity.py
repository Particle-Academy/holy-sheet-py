...
@pytest.fixture(params=sorted(SCHEMAS), ids=sorted(SCHEMAS))
def package(request) -> zipfile.ZipFile:
    return _package(SCHEMAS[request.param])
...
def test_relationship_ids_are_unique_within_a_part() -> None:
    ...
    Not hypothetical, and not far away: `dark-slide` — this package's pptx
    sibling — allocates image relationship ids from a GLOBAL media counter
    while a slide's own layout relationship is also `rId1`, so **every deck
    containing an image** emits a duplicate, in the PHP engine, the Node engine
    and now the Python one. The shape of that bug is one counter feeding two id
    spaces, and any writer that grows a second kind of relationship is one
    refactor away from it.
    """
    for name in sorted(SCHEMAS):
        package = _package(SCHEMAS[name])
        for part in package.namelist():
            if not part.endswith(".rels"):
                continue
            ids = [
