...
`package.json`, and `@particle-academy/holy-sheet` holds two constants that
...
def test_the_package_reports_the_version_it_ships_as() -> None:
    ...
    assert holy_sheet.version() == _declared_version(), (
...
def test_dunder_version_agrees_too() -> None:
    ...
    assert holy_sheet.__version__ == holy_sheet.version()


def test_the_version_is_a_semver_triple() -> None:
    assert re.fullmatch(r"\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?", holy_sheet.version())
...
def test_the_version_is_not_a_hardcoded_literal() -> None:
    ...
    to get wrong.
    """
    module = Path(holy_sheet.__file__)
